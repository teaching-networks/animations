/*
 * Copyright (c) Munich University of Applied Sciences - https://hm.edu/
 * Licensed under GNU General Public License 3 (See LICENSE.md in the repositories root)
 */

import 'image_info.dart';

class Images {
  static const computerIconImage = ImageInfo(path: "img/animation/computer.svg", aspectRatio: 415 / 289.5);
  static const hostIconImage = ImageInfo(path: "img/animation/host_icon.svg", aspectRatio: 232.28 / 142.6);
  static const routerIconImage = ImageInfo(path: "img/animation/router.svg", aspectRatio: 283.5 / 161);
  static const blueRouter = ImageInfo(path: "img/animation/blue_router.svg", aspectRatio: 283.5 / 161);
  static const germanyMapImage = ImageInfo(path: "img/animation/germany.svg", aspectRatio: 176.6 / 239.5);
  static const serverImage = ImageInfo(path: "img/animation/server.svg", aspectRatio: 245.5 / 233.7);
  static const key = ImageInfo(path: "img/animation/key.svg", aspectRatio: 355.4 / 157.8);
  static const database = ImageInfo(path: "img/animation/database.svg", aspectRatio: 283.5 / 337.9);
  static const scroll = ImageInfo(path: "img/animation/scroll.svg", aspectRatio: 166.8 / 202.4);
  static const cookie = ImageInfo(path: "img/animation/cookie.svg", aspectRatio: 1);
  static const cdmaCode = ImageInfo(path: "img/animation/cdma-code.svg", aspectRatio: 265.5 / 175.3);
  static const cdmaInput = ImageInfo(path: "img/animation/cdma-input.svg", aspectRatio: 306.5 / 156.9);
}
